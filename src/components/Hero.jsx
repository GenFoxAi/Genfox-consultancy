...
const Hero = ({ setIsModalOpen }) => {
  const handleScroll = () => {
    const toadSection = document.getElementById('toad');
    if (toadSection) {
      toadSection.scrollIntoView({ behavior: 'smooth' });
    }
  };
          ...
          <div className='px-2 sm:px-[15%] mt-10 flex flex-col items-start space-y-4 sm:flex-row sm:space-y-0 sm:space-x-4 md:space-x-5 lg:space-x-6 xl:space-x-6 sm:mt-8 md:mt-9 lg:mt-10 xl:mt-12'>
            ...
            <div className='flex flex-row space-x-4 gap-2 sm:space-x-0'>
              <button 
                ...
                className='bg-[#daeb20] text-black font-semibold py-3 px-8 rounded-lg cursor-pointer hover:scale-105 transition-colors w-full sm:w-auto sm:py-2 sm:px-6 md:py-3 md:px-7 lg:py-3 lg:px-8 xl:py-4 xl:px-9'
              >
            ...
            <div className='flex items-center space-x-4 text-gray-300 mt-4 sm:mt-0 sm:space-x-3 md:space-x-3 lg:space-x-4 xl:space-x-4'>
              ...
              <span 
                ...
                className='text-sm cursor-pointer w-10 h-10 flex justify-center items-center bg-[#8d9294] rounded-full text-white sm:w-8 sm:h-8 md:w-9 md:h-9 lg:w-10 lg:h-10 xl:w-12 xl:h-12 hover:bg-[#6b6f71] transition-colors'
              >
                ↓
              </span>
            </div>
